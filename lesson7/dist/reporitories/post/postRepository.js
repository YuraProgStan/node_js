...
let PostRepository = class PostRepository extends typeorm_1.Repository {
    async getPosts() {
        return (0, typeorm_1.getManager)().getRepository(post_1.Post).find({ relations: ['comments'] });
    }
    async getPostByUserId(userId) {
        return (0, typeorm_1.getManager)().getRepository(post_1.Post).findOne({ userId });
    }
    async updatePost(id, text) {
        return (0, typeorm_1.getManager)().getRepository(post_1.Post).update({ id }, { text });
    }
};
